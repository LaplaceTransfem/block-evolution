...
fn main(){
	...
	graph.connect("x",1_usize).with(Layer::linear(true,28*28,64,1.0));	
	graph.connect(1_usize,2_usize).with(Layer::relu());	
	graph.connect(2_usize,3_usize).with(Layer::linear(true,64,32,1.0));	
	graph.connect(3_usize,4_usize).with(Layer::relu());	
	graph.connect(4_usize,"y").with(Layer::linear(true,32,10,1.0));

	let y=Unvec(dbg!(&graph)).forward(x);													// apply the graph network by reference to x. AI trait contains the forward method, and is implemented for references too. Since graph can either take a hashmap or vector of inputs, letting it directly have single inputs caused type issues, so Unvec is a convenience wrapper for putting the input in a vec and taking the output out of the vec
	println!("{}",y.unwrap_f1());
}
